...
import com.example.biliapp.Manifest
...
class MainActivity : AppCompatActivity(), UpListAdapter.OnClickListener, UpListAdapter.OnLongClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.dynamicList.registerOnPageChangeCallback(object :
    ...
    override fun onLongClick(position: Int) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            ...
            intent.putStringArrayListExtra(
                "video_name",
                ArrayList(videoData[position].content!!.map { it.name })
            )
            intent.putIntegerArrayListExtra(
                "video_img",
                ArrayList(videoData[position].content!!.map { it.img })
            )
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            ...
            for (i in videoData) {
                outState.putStringArrayList(
                    "video_name_${i.owner}",
                    ArrayList(i.content!!.map { it.name })
                )
                outState.putIntegerArrayList(
                    "video_img_${i.owner}",
                    ArrayList(i.content.map { it.img })
                )
            }
        }
    }
    ...
    override fun onRestoreInstanceState(
        ...
        if (savedInstanceState != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                upData = savedInstanceState.getParcelableArrayList(
                    "up_data",
                    UpItem::class.java
                ) as ArrayList<UpItem>
                videoData = savedInstanceState.getParcelableArrayList(
                    "video_data",
                    IntermediateItem::class.java
                ) as ArrayList<IntermediateItem>
            } else {
